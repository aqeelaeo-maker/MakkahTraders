...
export default function Layout() {
  ...
  useEffect(() => {
    if (isAuthorized && user) {
      const unsub = onSnapshot(doc(db, 'companies', user.uid), (doc) => {
        if (doc.exists()) {
          const profileData = doc.data() as CompanyProfile;
          setCompanyProfile(profileData);
          
          // Check if we have an active subuser stored
          const storedSubUserStr = localStorage.getItem('activeSubUser');
          if (storedSubUserStr) {
             const storedSubUser = JSON.parse(storedSubUserStr) as SubUser;
             // Ensure it exists in the updated profile, unless subusers don't exist
             if (profileData.subUsers && profileData.subUsers.length > 0) {
               const valid = profileData.subUsers.find(u => u.username === storedSubUser.username && u.password === storedSubUser.password);
               if (valid) {
                 setActiveSubUser(valid);
               } else {
                 setActiveSubUser(null);
                 localStorage.removeItem('activeSubUser');
               }
             } else {
               setActiveSubUser({ id: 'user1', username: 'Owner', role: 'owner' });
             }
          } else {
             if (!profileData.subUsers || profileData.subUsers.length === 0) {
               setActiveSubUser({ id: 'user1', username: 'Owner', role: 'owner' });
  ...
  const handleSubUserLogin = (subUser: SubUser) => {
    localStorage.setItem('activeSubUser', JSON.stringify(subUser));
    setActiveSubUser(subUser);
  };

  const navItems: Array<{ name: string, path: string, icon: any, hideForStaff?: boolean }> = [
  ...
  if (user?.email?.toLowerCase() === 'aqeelaeo@gmail.com') {
    navItems.push({ name: 'Team', path: '/team', icon: Users, hideForStaff: true });
  }

  const visibleNavItems = navItems.filter(item => !(item.hideForStaff && activeSubUser?.role === 'staff'));

  if (!user || isAuthorized === null || (isAuthorized && !companyProfile && activeSubUser === null)) {
    return (
  ...
  if (companyProfile && companyProfile.subUsers && companyProfile.subUsers.length > 0 && !activeSubUser) {
    return <SubUserLogin companyProfile={companyProfile} onLogin={handleSubUserLogin} />;
  }
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 flex flex-col p-6 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto pt-6">
          <button
            onClick={companyProfile?.subUsers && companyProfile.subUsers.length > 0 ? handleSubUserLogout : handleLogout}
          ...
          >
            ...
            {companyProfile?.subUsers && companyProfile.subUsers.length > 0 ? 'Switch User' : 'Sign Out'}
          </button>
          <div className="bg-slate-800 p-4 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-white text-xs font-bold shrink-0">
                {(activeSubUser?.username || user.displayName || user.email || 'A')[0].toUpperCase()}
              </div>
              <div className="overflow-hidden">
                <p className="text-xs text-white font-bold truncate">{activeSubUser?.username || user.displayName || user.email}</p>
                <p className="text-[10px] text-slate-400 capitalize">{activeSubUser?.role || 'Admin User'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto">
          <Outlet context={{ companyProfile, activeSubUser }} />
        </div>
      </main>
